...
def build_broadcast_script(spy_phonetics: bool = False, tone_freq: float = 1000.0,
                            tone_duration: float = 1.5, tone_at=("interval", "pass-break", "signoff")):
    """
    Returns (script, grouped) where script is a list of (label, kind, payload)
    triples in broadcast order:
      kind == "speech" -> payload is text for kokoro to speak
    ...
    script = [("announcement", "speech", TEST_ANNOUNCEMENT)]
    ...
    script.append(("message_pass_1", "speech", message_spoken))
    if "pass-break" in tone_at:
        script.append(("pass_break", "tone", tone))
    script.append(("message_pass_2", "speech", message_spoken))
...
def render_audio(script, out_stem="omaradio_numbers_station", formats=("wav",),
    ...
    for label, kind, payload in script:
        ...
        if pause is None:
            pause = np.zeros(int(sample_rate * 0.6), dtype=samples.dtype)
        chunks.append(samples.astype(pause.dtype, copy=False))
        chunks.append(pause)
...
def main():
    ...
    print(decoded)
    assert decoded == HIDDEN_MESSAGE.upper(), "Round-trip mismatch -- cipher tables out of sync."
