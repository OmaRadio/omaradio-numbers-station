...
OmaRadio sometimes operates as a number station to send out clandestine messages to Omarchy operatives worldwide. The scripts here can generate the broadcast audio including an "Announcement" message & "Message File" which gets transformed to encoded number form. The supplied written text is passed through Kokoro TTS.
...
Kokoro model weights -- a SEPARATE one-time download. Run once, in the same directory as this script (or pass
...
The script should install these on first run if not already installed.
...
`uv run omaradio_numbers_station_pause.py --audio --format mp3 --degrade heavy --voice bm_daniel --message-speed 0.92 --digit-pause 0.20`
